package sample

import (
	"math/rand"
	pcbook "pcbook/proto"

	"github.com/google/uuid"
)

func randomBool() bool {
	return rand.Intn(2) == 1
}

func randomKeyboardLayout() pcbook.Keyboard_Layout {
	switch rand.Intn(3) {
	case 1:
		return *pcbook.Keyboard_QWERTY.Enum()
	case 2:
		return *pcbook.Keyboard_AZERTY.Enum()
	default:
		return *pcbook.Keyboard_AZERTY.Enum()
	}
}

func randomCPUBrand() string {
	return randomStringFromSet("Intel", "AMD")
}

func randomStringFromSet(a ...string) string {
	n := len(a)
	if n == 0 {
		return ""
	}
	return a[rand.Intn(n)]
}

func randomCPUName(brand string) string {
	if brand == "Intel" {
		return randomStringFromSet(
			"Xeon E-2285M",
			"Core i9-9980HK",
			"Core i5-9400F",
			"Core i3-1005G1",
		)
	}
	return randomStringFromSet(
		"Ryzen 7 PRO 2700U",
		"Ryzen 5 PRO 3500U",
		"Ryzen 3 PRO 3200GE",
	)
}

func randomInt(min int, max int) int {
	return min + rand.Intn(max-min+1)
}

func randomFloat64(min, max float64) float64 {
	return min + rand.Float64()*(max-min+1)
}

func randomFloat32(min, max float32) float32 {
	return min + rand.Float32()*(max-min+1)
}

func randomGPUBrand() string {
	return randomStringFromSet("NVIDIA", "AMD")
}

func randomGPUName(brand string) string {
	if brand == "NVIDIA" {
		return randomStringFromSet(
			"RTX 2060",
			"RTX 2070",
			"GTX 1660-Ti",
			"GTX 1070",
		)
	}
	return randomStringFromSet(
		"RX 590",
		"RX 580",
		"RX 5700-XT",
		"RX Vega-56",
	)
}

func randomScreenPanel() pcbook.Screen_Panel {
	if rand.Intn(2) == 1 {
		return pcbook.Screen_IPS
	}
	return pcbook.Screen_OLED
}

func randomID() string {
	return uuid.New().String()
}

func randomLaptoBrand() string {
	return randomStringFromSet("Dell", "Apple", "Lenovo")
}

func randomLaptopName(brand string) string {
	switch brand {
	case "Apple":
		return randomStringFromSet("Macbook air", "Macbook pro")
	case "Dell":
		return randomStringFromSet("Latitude", "Vestro", "XPS", "Alienware")
	default:
		return randomStringFromSet("Thinkpad XI", "Thinkpad P1", "Thinkpad P53")
	}
}
